...
module.exports.run = async (client, oldMember, newMember) => {
  if(!client.channels.cache.get(uptimerChannel)) return
  if (oldMember.roles.cache.size !== newMember.roles.cache.size) {
    if (!oldMember.roles.cache.has(boosterRole) && newMember.roles.cache.has(boosterRole)) {
      let count = 0;
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(`Congratulations, you've got 20 coins to add projects in Alive bot.\n Add one using: \`.add <url>\`\nCheck Coins: \`.balance\``)
